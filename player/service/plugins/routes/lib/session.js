...
module.exports = Session = {
  // Param db was originally destructured a.k.a. {db}, does it need to be?
  // Causing tests to fail, also removing tenantId, it's not used.
  ...
  tryGetQueryResult: async (txn, sessionId, tenantId) => {
    try {
      ...
      await txn.rollback();
      throw new Error(
        `Failed to select session, registration course AU, registration and course AU for update: ${ex}`
      );
    }
  },
  ...
  getQueryResult: async (txn, sessionId, tenantId) => {
    return await txn
      ...
      .leftJoin(
        "registrations",
        "registrations_courses_aus.registration_id",
        "registrations.id"
      )
      .leftJoin(
        "courses_aus",
  ...
  abandon: async (sessionId, tenantId, by, { db, lrsWreck }) => {
    ...
    try {
      ({ session, regCourseAu, registration, courseAu } =
    ...
    [stResponse, stResponseBody] = await Session.retrieveResponse(
      durationSeconds,
      session,
      regCourseAu,
      registration,
  ...
  initializeDuration(session) {
    ...
    if (session.is_initialized) {
      durationSeconds =
        (new Date().getTime() - session.initialized_at.getTime()) / 1000;
    }
  ...
  retrieveResponse: async (
    ...
    try {
      (stResponse = await lrsWreck.request("POST", "statements", {
        headers: {
      ...
      await txn.rollback();
      throw Boom.internal(
        new Error(`Failed request to store abandoned statement: ${ex}`)
      );
    }
  },
  ...
  checkStatusCode: async (txn, stResponse, stResponseBody) => {
    if (stResponse.statusCode !== 200) {
      ...
      throw Boom.internal(
        new Error(
          `Failed to store abandoned statement (${stResponse.statusCode}): ${stResponseBody}`
        )
      );
    }
  },
  ...
  txnUpdate: async (session, tenantId, txn, by) => {
    try {
      await txn("sessions")
        .update({ is_abandoned: true, abandoned_by: by })
