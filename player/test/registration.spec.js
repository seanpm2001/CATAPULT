...
describe('Test of create function', async function () {
	var tenantId =0, courseId =0, actor, lrsWreck, registrationId, registration, courseAUs, createStub, testCreate, error
	var createSpy, getCourseStub, getCourseAUsStub, updateCourseAUmapStub, parseRegistrationDataStub, updateMetadataStub
	const code = 1000
...
var txn = knex({
	...
	const db = {
		transaction: async () => {
			...
			var regResult = await txn('registrations').insert(registration)
...
describe('Test of load function', function () {
  var loadSpy, loadRegistrationStub, loadRegistrationAusStub, registrationTest, error
  var tenantId, registrationId, course
  var loadAus = true;
  const registration = {
    aus: null
  }
...
describe('Test of loadRegistration', function () {
	...
	var tenantId, registrationId, queryResult
...
describe('Test of loadAuForChange function', function () {
	var loadAuForChangeSpy, getQueryResultStub, tenantId, registrationId, auIndex, queryTest, regCourseAu, error
	var queryResult ={
...
describe('Test of updateMetaData function', function () {
	...
	var updateMetadataSpy, testQuery
	var tenantId = 0;
	var registration = {
		id: 0,
		metadata: ''
...
describe('Test of updateCourseAUmap', function () {
  ...
  let tenantId, registrationId, updateResult
  var courseAUs = {
	  map: () => { return true}
  } 
  var txn = knex ({
